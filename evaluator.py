...
class StabilityEvaluator(Evaluator):
    def __init__(self, args):
        super(StabilityEvaluator, self).__init__(args)
    ...
    def forward(
        self, prompt_pre="",  eval_src=None, candidate_responses=None, output=None
    ):
        scores1 = self.get_judge_scores(prompt_pre, eval_src, candidate_responses) # make this parallel
        scores2 = self.get_judge_scores(prompt_pre, eval_src, candidate_responses) #

        stability_score = self.get_stability(scores1, scores2)

        print(f"Stability score: {stability_score}")
        return {"hypos": scores1, "scores": [stability_score]}
    
    def get_judge_scores(self, prompt_pre, eval_src, candidate_responses):
        scores = []

        # SystemPrompt: Please act as an impartial judge and evaluate the quality of the response provided by an AI assistant to the user question displayed below. Your evaluation should consider factors such as the helpfulness, relevance, accuracy, depth, creativity, and level of detail of the response. You evaluation should focus on the assistant's answer to the second user question. Be as objective as possible.
...
Score: """
        ...
        judge_dataset = []
        for question, response in zip(eval_src, candidate_responses):
            judge_prompt = scoring_template.format(question=question, response=response)
            judge_dataset.append(judge_prompt)
        
        print('Judge dataset len:', len(judge_dataset))

        # Use batch processing to get scores
        batch_size = self.args.batch_size
        for i in range(0, len(judge_dataset), batch_size):
            batch = judge_dataset[i:i+batch_size]
            batch_scores = llm_query(
                batch,
                client=self.client,
            ...
            for score in batch_scores:
                try:
                    ...
                    if 1 <= numeric_score <= 10:
                        ...
                        self.logger.warning(f"Invalid score: {score}. Using default score of 0.")
                        scores.append(0.0)
                except ValueError:
                    self.logger.warning(f"Could not convert '{score}' to float. Using default score of 0.")
                    scores.append(0.0)
        print('Scores:', scores)
        return scores
    
    def get_stability(self, hypos1, hypos2):
        #num matches/total * 100
        stability_score = 0
        for i in range(len(hypos1)):
            if hypos1[i] == hypos2[i]:
                stability_score += 1
        stability_score = (stability_score/len(hypos1)) * 100
